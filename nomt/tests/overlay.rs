...
fn overlay_deletions() {
    let test_db = || -> Test {
        ...
        test.write(vec![0; 32], Some(vec![1, 1]));
        test.write(vec![1; 32], Some(vec![2, 2]));

        // subtree at 001000_00/01/10
        test.write(vec![32; 32], Some(vec![1, 1]));
        test.write(vec![33; 32], Some(vec![2, 2]));
        test.write(vec![34; 32], Some(vec![3, 3]));

        // subtree at 100000_00/01/10/11
        test.write(vec![128; 32], Some(vec![4, 4]));
        test.write(vec![129; 32], Some(vec![5, 5]));
        test.write(vec![130; 32], Some(vec![6, 6]));
        test.write(vec![131; 32], Some(vec![7, 7]));
    ...
    test.write(vec![0; 32], None);
    test.write(vec![32; 32], None);
    ...
    assert_eq!(test.read(vec![33; 32]), Some(vec![2, 2]));
    assert_eq!(test.read(vec![34; 32]), Some(vec![3, 3]));

    assert_eq!(test.read(vec![128; 32]), None);
    assert_eq!(test.read(vec![129; 32]), Some(vec![5, 5]));
    assert_eq!(test.read(vec![130; 32]), Some(vec![6, 6]));
    assert_eq!(test.read(vec![131; 32]), Some(vec![7, 7]));
    ...
    test.write(vec![1; 32], None);
    test.write(vec![33; 32], None);
    ...
    assert_eq!(test.read(vec![32; 32]), Some(vec![1, 1]));
    assert_eq!(test.read(vec![33; 32]), None);
    assert_eq!(test.read(vec![34; 32]), Some(vec![3, 3]));

    assert_eq!(test.read(vec![128; 32]), Some(vec![4, 4]));
    assert_eq!(test.read(vec![129; 32]), None);
    assert_eq!(test.read(vec![130; 32]), Some(vec![6, 6]));
    assert_eq!(test.read(vec![131; 32]), Some(vec![7, 7]));
    ...
    assert_eq!(test.read(vec![129; 32]), None);
    assert_eq!(test.read(vec![130; 32]), Some(vec![6, 6]));
    assert_eq!(test.read(vec![131; 32]), None);
...
fn overlay_deletions_respected_in_seek() {
    ...
    test.write(key_a.clone(), Some(vec![42]));
    test.write(key_b.clone(), Some(vec![43]));
    test.write(key_c.clone(), Some(vec![44]));
    let _ = test.commit();
...
fn overlay_earlier_deletions_respected_in_seek() {
    ...
    test.write(key_b.clone(), Some(vec![42]));
    test.write(key_c.clone(), Some(vec![43]));
    test.write(key_d.clone(), Some(vec![44]));
    let _ = test.commit();
