...
pub struct NomtIterator<'a> {
    overlay_key_value_iter: Peekable<Box<dyn Iterator<Item = (KeyPath, &'a ValueChange)> + 'a>>,
    key_value_iter: Peekable<KeyValueIterator>,
}

impl<'a> NomtIterator<'a> {
    ...
    pub(crate) fn new(
        ...
        let key_value_iter = read_transaction
            ...
            .peekable();
        let overlay_key_value_iter: Box<dyn Iterator<Item = (KeyPath, &'a ValueChange)> + 'a> =
            Box::new(overlay.value_iter(start, end));
        let overlay_key_value_iter = overlay_key_value_iter.peekable();
...
impl<'a> Iterator for NomtIterator<'a> {
    type Item = (KeyPath, Value);

    fn next(&mut self) -> Option<Self::Item> {
        let extract_value = |v: &ValueChange| -> Vec<u8> {
            match v {
                ValueChange::Insert(val) | ValueChange::InsertOverflow(val, _) => val.clone(),
